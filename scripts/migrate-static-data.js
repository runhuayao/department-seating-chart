...
async function migrateDepartments() {
  ...
  try {
    ...
    for (const dept of staticData.departments) {
      // 插入部门基本信息
      await client.query(`
        INSERT INTO departments (name, display_name, description, floor, building, map_data)
        VALUES ($1, $2, $3, $4, $5, $6)
      ...
      `, [
        dept.name,
        dept.map_data.map_id,
        dept.map_data.type,
        dept.map_data.url,
        dept.map_data.dept_name
      ]);
    }
...
async function migrateEmployees() {
  ...
  try {
    ...
    for (const emp of staticData.employees) {
      await client.query(`
        INSERT INTO employees (employee_id, name, department, status)
        VALUES ($1, $2, $3, $4)
        ON CONFLICT (employee_id) DO UPDATE SET
          ...
          updated_at = CURRENT_TIMESTAMP
      `, [emp.employee_id, emp.name, emp.department, emp.status]);
    }
    ...
    for (const desk of staticData.desks) {
      await client.query(`
        ...
        ON CONFLICT (desk_id) DO UPDATE SET
          ...
          status = CASE WHEN EXCLUDED.employee_id IS NOT NULL THEN 'occupied' ELSE 'available' END,
          updated_at = CURRENT_TIMESTAMP
      `, [
        desk.desk_id,
        desk.label,
  ...
  try {
    ...
    await client.query(`
      UPDATE departments SET 
        total_desks = desk_counts.total,
        occupied_desks = desk_counts.occupied,
        updated_at = CURRENT_TIMESTAMP
      FROM (
        SELECT 
          department,
          COUNT(*) as total,
          COUNT(employee_id) as occupied
        FROM desk_positions 
        GROUP BY department
      ) AS desk_counts
      WHERE departments.name = desk_counts.department
    `);
    
    console.log('✅ 部门统计信息更新完成');
  } catch (error) {
